...
export function fetchApi(target: ApisName, evolution = false): Promise<any> {
  const apiData = listUrls.find(({ name }) => name === target);
  if (!apiData) return;
  return fetch(`https://docs.google.com/spreadsheets/d/e/${apiData.id}/pub?gid=${evolution ? apiData.evolgid : apiData.gid}&single=true&output=csv`).then(response =>
    response.text(),
  );
}
