...
const CaUniq = class {
  ...
  async getUniqs() {
    var _a;
    (_a = utils.fetchApi(utils.ApisName.Uniq)) === null || _a === void 0 ? void 0 : _a.then(data => {
      var _a;
      this.uniqs = utils.csvToJson(data);
      (_a = utils.fetchApi(utils.ApisName.Set)) === null || _a === void 0 ? void 0 : _a.then(data => (this.uniqs = [...this.uniqs, ...utils.csvToJson(data)])).then(data => {
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "close" }), index.h("div", { class: "header" }, "HOLY GRAIL [", this.holyGrail.length, "/", this.uniqs.length, "] (\uD83C\uDFC6", Math.round((this.holyGrail.length / this.uniqs.length) * 10000) / 100, "%)"), index.h("div", { class: "actions" }, index.h("input", { list: "list-uniq", value: this.search, onInput: event => {
        this.handleChange(event);
      }, placeholder: "Item name" }), index.h("div", { class: {
        on: this.searchHoly,
        action: true,
      }, onClick: () => (this.searchHoly = !this.searchHoly) }, "\uD83C\uDFC6"), index.h("div", { class: {
        on: this.listening,
        action: true,
      }, onClick: () => {
        if (this.listening) {
      ...
      } }, "\uD83C\uDFA4")), index.h("div", { class: "list" }, this.filterList(this.uniqs).map(({ id, value, ['lvl req']: lvl, type }) => {
      const holyGrailed = this.isHolyGrail(id);
      return (index.h("div", { key: id, class: {
          grailed: !!holyGrailed,
          uniq: true,
        }, onClick: () => this.setHolyGrail(id) }, this.displayimg(id, type), index.h("div", { class: "infos" }, index.h("div", { class: "name" }, id), index.h("div", { class: "type" }, type), index.h("div", { class: "lvl" }, lvl)), index.h("div", { class: "value" }, value), index.h("div", { class: "holyGrail" }, holyGrailed ? (holyGrailed.ethereal ? '🏆<Ethereal>' : '🏆') : '')));
    }))));
  }
  reduceType(target) {
  ...
  manageErrorImg(ev, type) {
    ...
    const typesrc = index.getAssetPath(`./assets/img/${typetrim}.png`);
  ...
  displayimg(id, type) {
    ...
    return (index.h("div", { class: "img" }, index.h("img", { onError: ev => this.manageErrorImg(ev, type), src: index.getAssetPath(`./assets/img/${idTrim}.png`), loading: "lazy" })));
  }
